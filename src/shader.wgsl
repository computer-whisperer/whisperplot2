...
struct GridLevelUniform {
    // Single sequence of ticks: first + i*step
    first: f32,
    step: f32,
    count: u32,
...
fn vs_grid(
    ...
    if (u_grid.orient == 0u) {
        ...
        let x_tick = u_grid.first + idx * u_grid.step;
        ...
        let y_tick = u_grid.first + idx * u_grid.step;
...
fn fs_grid(in: VertexOutput) -> @location(0) vec4<f32> {
    // Neutral grid color; alpha driven by level planning
    return vec4<f32>(0.6, 0.6, 0.6, u_grid.alpha);
}